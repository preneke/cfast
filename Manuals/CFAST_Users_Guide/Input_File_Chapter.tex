...
\item[Cross-sectional Area] (default units: \unit{m^2}, default value: 0~\unit{m^2})
\label{Ceil Shape}
...
\item[Area] (default units: \unit{m^2}, default value: 0~\unit{m^2}): Cross-sectional area of the opening.
\label{Mech Height}
\item[Center Height] (default units: \unit{m}, default value: 0~\unit{m}): Height of the midpoint of the duct opening above the floor.
...
\item[Vent Offset] (default units: \unit{m}, Default value: 0~\unit{m}): For visualization only, the horizontal distances between the center of the vent and the origin of the X and Y axes in the first compartment. See figure \ref{fig:compartment_positioning} for axis position conventions in CFAST.
\end{description}
...
\item[Flow Rate] (default units: \unit{m^3/s}, default value: 0~\unit{m^3/s}): Constant flow rate of the fan.
\label{Mech Begin Drop Off Pressure}
\item[Begin Drop Off Pressure] (default units: \unit{Pa}, default value: 200~\unit{Pa}): Above this pressure, the flow begins a drop-off to zero. A hyperbolic tangent function is used to ensure a smooth transition from full flow at the ``Begin Drop Off Pressure'' to zero flow at the ``Zero Flow Pressure''.
\label{Mech Zero Flow Pressure}
\item[Zero Flow Pressure] (default units: \unit{Pa}, default value: 300~\unit{Pa}): The pressure above which the flow is zero.
\end{description}
...
\item[\texttt{SETPOINT}] Trigger time in s, temperature in \unit{\degreeCelsius}, or incident heat flux in \unit{kW/m^2}, as selected by \ct{CRITERION}.
...
\begin{description}
\item[\texttt{AREA}] Required vent area in \unit{m^2}.
...
\begin{description}
\item[\texttt{AREAS}] Required diffuser areas in \unit{m^2} at the two ends of the duct.
\item[\texttt{CUTOFFS}] Pressure differences in Pa at which flow begins to decrease and reaches zero; defaults 200 and 300~\unit{Pa}.
...
\item[\texttt{FLOW}] Required nominal volumetric flow rate in \unit{m^3/s}.
